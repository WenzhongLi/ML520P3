...
'''
import generater
import Hunter_part1_HP_MA
import Hunter_part1_HPF_MA
import random
...
for type in range(0, 4):
    for i in range(0,test_num):
        ...
        hunter_rule1 = Hunter_part1_HP_MA.hunter_HCT(matrix, size, True)
        count_rule1[type] += hunter_rule1.find_target(target_node)

        hunter_rule2 = Hunter_part1_HPF_MA.hunter_HCT(matrix, size, True)
    ...
    for i in range(0, test_num):
        ...
        for m in range(0, size):
            for n in range(0, size):
                node_map_by_type[matrix[m][n]].append((m, n))
        target_node = random.choice(node_map_by_type[type])
        hunter_rule1 = Hunter_part1_HP_MA.hunter_HCT(generator.get_matrix(), size, True)
        count_rule1_diff_map[type] += hunter_rule1.find_target(target_node)

        hunter_rule2 = Hunter_part1_HPF_MA.hunter_HCT(generator.get_matrix(), size, True)
